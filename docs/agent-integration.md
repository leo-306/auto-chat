...
本文档说明 Codex 或其他 agent 如何与本项目的 ChatGPT 生图自动化系统交互。
...
插件默认处于暂停状态，不会自动 claim 队列任务。点击 popup 的“立即领取一轮”会执行一次手动调度；点击“开启自动执行”才会持续自动领取队列任务。
...
插件按 ChatGPT 生图卡片顺序采集，并按 estuary `file_...` 去重。同一张图片可能在 DOM 中出现多次，不能直接按 `<img>` 数量判断。
