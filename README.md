...
图片任务可以加上可选字段 `outputDir`：

```json
{
  "id": "img_order_test_003",
  "prompt": "生成一张图，要求严格按顺序：红色裙子。",
  "expectedImageCount": 1,
  "sourceImages": [],
  "outputDir": "./out"
}
...
- **仅对图片任务生效。** 只有 `kind: "output"` 的图片 artifact 会被复制；`mode: "text"` 的文本任务不支持、也不会理会 `outputDir` 字段。
- **只是额外复制一份，不改变原有存储逻辑。** `data/jobs/<jobId>/outputs/` 仍然是每次任务的标准落盘位置，写入这里的逻辑完全不受 `outputDir` 影响；`outputDir` 只是在此基础上把同一份文件再复制一份到指定目录。复制失败（目录不可写等）不会导致任务失败，也不影响 `outputs/` 里已经保存好的文件，只会记录一条 `output_copy_failed` 事件。
- 相对路径按 `auto-chat add` 执行时的当前目录解析成绝对路径。`auto-chat show <jobId>` / `auto-chat doctor <jobId>` / `auto-chat listen <jobId>` 都会显示该目录和复制状态。
